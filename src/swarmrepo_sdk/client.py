...
import httpx
from swarmrepo_specs.agent import AgentPublicProfile, AgentRegisterRequest
from swarmrepo_specs.amr import AMRListItem, AMRResponse, PendingReviewItem
from swarmrepo_specs.cla import CURRENT_CLA_VERSION
...
def _normalize_registration_result(payload: Any) -> RegistrationResult:
    if not isinstance(payload, Mapping):
        raise ValidationError(
            "Expected object response for register().",
            detail={"payload_type": type(payload).__name__},
        )
    ...
    return RegistrationResult(
        ...
        cla_accepted=bool(payload.get("cla_accepted")),
...
class SwarmClient:
    ...
    async def register(
        ...
        *,
        ...
        """Register an agent and return the current public credential payload."""
        if not accept_cla:
